...
| # | 測試項目 | 測試方法 | 預期結果 | 狀態 |
|---|---------|---------|---------|------|
| 8.1 | fetch CSV 成功 | browser console / network | CSV 數據成功加載 | ⬜ |
| 8.2 | parseCSV 正確解析 | browser console | 數據正確轉為物件 | ⬜ |
| 8.3 | 顯示正確既功課內容 | snapshot | 日曆顯示功課項目 | ⬜ |

#### 日期邏輯測試

| # | 測試項目 | 測試方法 | 預期結果 | 狀態 |
|---|---------|---------|---------|------|
| 8.4 | 星期日header紅色 | snapshot | 星期日顯示紅色 | ⬜ |
| 8.5 | 星期六header藍色 | snapshot | 星期六顯示藍色 | ⬜ |
| 8.6 | 上一週背景灰色 | snapshot | 過去日期顯示灰色背景 | ⬜ |
| 8.7 | 今日highlight | snapshot | 今日有明顯標記 | ⬜ |
| 8.8 | 截止前一日顯示提醒 | snapshot | 截止前一日有提醒標記 | ⬜ |

#### 自動化測試 (Optional)

| # | 測試項目 | 測試方法 | 預期結果 | 狀態 |
|---|---------|---------|---------|------|
| 8.9 | n8n webhook接收圖片 | 發送測試圖片 | webhook 收到請求 | ⬜ |
| 8.10 | AI OCR辨識功課 | 上傳功課圖片 | 正確提取文字 | ⬜ |
| 8.11 | 自動寫入Google Sheets | 查看 Sheets | 數據自動更新 | ⬜ |
| 8.12 | 網頁自動更新 | 定時刷新 | 數據自動刷新 | ⬜ |
...
## 🔨 開發者檢查清單 (REFERENCE/CHECKLIST.md)
